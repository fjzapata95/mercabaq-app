...
import { ProductCreateResponse } from '@core/interfaces/products.interfaces';
...
export const CreateProductScreen = (props: Props) => {
    ...
    const onCreate = useCallback(async () => {
        ...
        if (validator.valid) {
            ...
            try {
                const { data: { data, error, message } } = await apiHelpers.post<ProductCreateResponse>('producto/create', form);
                // VALIDAR 
                if ( error ) {
                ...
                if (error && error.data && error.data.message) {
                    ...
                    // MENSAJE POR DEFECTO
                    dispatch(showAlert({show: true, message: 'Lo sentimos, pero no pudimos completar la creación de su producto en este momento. Por favor, verifica tus datos e intenta nuevamente.'}));
                }
                setIsSubmitting(false);
                ...
                >
                    <View style={styles.body}>
                        ...
                        >
                            ...
                            > 
                                <View style={styles.itemBody}>
                                    ...
                                    <View style={{paddingHorizontal: 8, marginBottom: 10, alignSelf: 'center'}}>
                                        {/* BUTTON SUBMIT */}
                                        <ButtonSubmit mode="contained" loading={isSubmitting} disabled={isSubmitting} onPress={onCreate}>{'Guardar cambios'}</ButtonSubmit>
                                    </View>
                                </View>
                            </List.Accordion>
                            ...
                            >
                                <View style={styles.itemBody}>
                                    <ImageProduct product={product} handlePress={handlePress} />
                                </View>
                            </List.Accordion>
                            ...
                            >
                                <View style={styles.itemBody}>
                                    <MainImage product={product} navigation={props.navigation} reset={reset} handlePress={handlePress} />
                                </View>
                            </List.Accordion>
                        </List.AccordionGroup>
                    </View>
